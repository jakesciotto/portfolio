...
const NUMBER =
  'block mt-3 text-[40px] leading-none font-bold font-mono tracking-tighter'
  ...
  return (
    <>
      <span className={`flex items-center gap-2 ${LABEL}`}>
        <i
          className={`inline-block h-[7px] w-[7px] rounded-[2px] ${bgClass[accent]}`}
        />
        {label}
      </span>
      <span
        className={`${NUMBER} ${zero ? 'text-muted-foreground/60' : textClass[accent]}`}
      >
        {count}
      </span>
      <div className="mt-auto pt-3.5">
        <div className="h-[3px] overflow-hidden rounded-sm bg-border-strong">
          <div
            className={`h-full rounded-sm ${bgClass[accent]}`}
            style={{ width: `${pct}%` }}
          />
        </div>
...
function SummaryBody({ active, overdue, lastSync }) {
  return (
    <>
      ...
      <span className={`${NUMBER} mt-2 text-foreground`}>
        {active ?? '---'}
      </span>
      ...
      {overdue > 0 && (
        <span className="mt-2 font-mono text-xs font-semibold text-accent-red">
          {overdue}{' '}
          <span className="font-medium text-muted-foreground/70">overdue</span>
        </span>
      )}
