  ...
  return (
    <span className="flex shrink-0 items-center" aria-hidden={ariaHidden}>
      {items.map((it, i) => (
        <span
          key={`${it.playedAt}-${i}`}
          className="flex items-center whitespace-nowrap pr-10 text-sm"
        >
    ...
    <div className="flex h-full flex-col">
      <div className="mb-3 flex items-center justify-between">
        ...
        <span className={LABEL}>
          last {items.length} plays
          {items[0]?.playedAt && ` · latest ${agoLabel(items[0].playedAt)}`}
        </span>
      </div>
      ...
      {data.live?.minutes > 0 && (
        <p
          ...
          title="minutes collected live since the collector started"
        >
